 ...
 * @author Winfxk
 */
public class SoarTo extends BaseSecondary {
	...
	public boolean MakeSecondary() {
		...
		String Contxt = msg.getSun("修复商店", "定值修复", "内容", myKey, myData);
		...
		for (Integer i : set) {
			...
			if (Tool.ObjToBool(Item.get("isTool"), false) || item.isArmor() || item.isTool()) {
				list.add(msg.getSun("修复商店", "定值修复", "列表格式", listK, new Object[] { player.getName(), data.MyMoney(),
	...
	public boolean disSecondary(FormResponseCustom data) {
		...
		Map<Integer, cn.nukkit.item.Item> Contents = Inventory.getContents();
		cn.nukkit.item.Item item = Contents.get(i);
		item.setDamage(Repair);
		Contents.put(i, item);
		Inventory.setContents(Contents);
		return this.data.send(
				msg.getSun("修复商店", "定值修复", "购买成功", new String[] { "{Player}", "{MyMoney}", "{Money}", "{reduceMoney}" },
